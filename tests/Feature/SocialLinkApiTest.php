...
/**
 * `social-links` is readable without a token - it is what the public
...
class SocialLinkApiTest extends TestCase
{
    ...
    public function test_social_links_index_is_public_to_a_caller_that_names_its_tenant(): void
    {
        $tenant = Tenant::factory()->create();

        $this->getJson('/api/v1/social-links', ['X-Tenant-ID' => $tenant->slug])
            ->assertOk();
    }

    public function test_social_links_index_is_refused_when_no_tenant_can_be_resolved(): void
    {
        $this->getJson('/api/v1/social-links')
            ->assertStatus(400);
    }

    public function test_social_links_index_returns_200_for_an_authenticated_user(): void
    {
        $tenant = Tenant::factory()->create();
