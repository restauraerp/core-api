...
/**
 * `website-settings` is readable without a token - it is what the public
 ...
 */
class WebsiteSettingApiTest extends TestCase
{
    use RefreshDatabase;

    public function test_website_settings_index_is_public_to_a_caller_that_names_its_tenant(): void
    {
        $tenant = Tenant::factory()->create();

        $this->getJson('/api/v1/website-settings', ['X-Tenant-ID' => $tenant->slug])
            ->assertOk();
    }

    public function test_website_settings_index_is_refused_when_no_tenant_can_be_resolved(): void
    {
        $this->getJson('/api/v1/website-settings')
            ->assertStatus(400);
    }

    public function test_website_settings_index_returns_200_for_an_authenticated_user(): void
    {
        $tenant = Tenant::factory()->create();
        ...
        $this->getJson('/api/v1/website-settings')->assertOk();
    }
}
